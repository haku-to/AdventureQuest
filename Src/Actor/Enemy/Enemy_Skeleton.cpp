...
enum
{
	...
	MotionDown = 4,		  //ダウンモーション
	MotionDamage = 6,	  //ダメージモーション
	MotionIdle = 8,		  //アイドルモーション
	MotionWalk = 9,		  //歩きモーション
	MotionVictory = 14,	  //勝利モーション
};
...
//コンストラクタ
Enemy_Skeleton::Enemy_Skeleton(IWorld* world, const GSvector3& position, float angle) :
	mesh_{ Mesh_Skeleton,Mesh_Skeleton,Mesh_Skeleton,MotionIdle },
	motion_{ MotionIdle },
	motion_loop_{ true },
	state_{ State::Idle },
	state_timer_{ 0.0f },
...
//更新
void Enemy_Skeleton::update(float delta_time) {
	//プレイヤーを検索
...
//描画
void Enemy_Skeleton::draw()const {
	//メッシュの描画
	mesh_.draw();
}

//衝突リアクション
void Enemy_Skeleton::react(Actor& other) {
	...
	{
		SE::PlaySE(SE_Enemy_Skeleton_Damage);
		...
		{
			//スケルトンの死亡音を出す
			SE::PlaySE(SE_Enemy_Skeleton_Down);
			//ダウンモーションに移る
...
//状態更新
void Enemy_Skeleton::update_state(float delta_time) {
	//各状態に分岐する
	switch (state_) {
...
//ダメージ中
void Enemy_Skeleton::damage(float delta_time) {
...
//死亡
void Enemy_Skeleton::down(float delta_time) {
	if (state_timer_ >= mesh_.motion_end_time()) {
		//モーションが終了したら死亡
		die();
	}
...
float Enemy_Skeleton::target_signed_angle()const {
	//プレイヤーがいなければ何もしない
...
float Enemy_Skeleton::target_angle() const {
	return std::abs(target_signed_angle());
}

//ターゲットとの距離を求める
float Enemy_Skeleton::target_distance()const {
	//ターゲットとの距離を計算する
...
//フィールドとの衝突判定
void Enemy_Skeleton::collide_field() {
	//壁との衝突判定(球体との判定）
...
//アクターとの衝突処理
void Enemy_Skeleton::collide_actor(Actor& other) {
	//y座標を除く座標を求める
	GSvector3 position = transform_.position();
	position.y = 0.0f;
...
//状態の変更
void Enemy_Skeleton::change_state(State state, GSuint motion, bool loop) {
	//モーション番号の更新
	motion_ = motion;
	//モーションループ指定
	motion_loop_ = loop;
...
//アイドル状態
void Enemy_Skeleton::idle(float delta_time) {
	// 攻撃するか？
	if (is_attack())
	{
		// 攻撃状態に遷移
...
//移動中
void Enemy_Skeleton::walk(float delta_time) {
	// ターゲット方向の角度を求める
...
//振り向き中
void Enemy_Skeleton::turn(float delta_time) {
	if (state_timer_ >= mesh_.motion_end_time())
	{
...
//攻撃中
void Enemy_Skeleton::attack(float delta_time) {
...
//ターンしているか
bool Enemy_Skeleton::is_turn()const {
	return(target_distance() <= TurnDistance);
}

//攻撃しているか
bool Enemy_Skeleton::is_attack()const {
	return (target_distance() <= AttackDistance) && (target_angle() <= 20.0f);
}

//歩いているか
bool Enemy_Skeleton::is_walk()const {
	return(target_distance() <= WalkDistance);
}

//攻撃判定生成
void Enemy_Skeleton::generate_attack_collider() {
	//攻撃判定を出現させる場所の距離
