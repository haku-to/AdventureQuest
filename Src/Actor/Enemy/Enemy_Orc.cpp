...
//コンストラクタ
Enemy_Orc::Enemy_Orc(IWorld* world, const GSvector3& position, float angle) :
	mesh_{ Mesh_Orc,Mesh_Orc,Mesh_Orc,MotionIdle },
	motion_{ MotionIdle },
	motion_loop_{ true },
	state_{ State::Idle },
	state_timer_{ 0.0f },
...
//更新
void Enemy_Orc::update(float delta_time) {
	//プレイヤー検索
...
//描画
void Enemy_Orc::draw()const {
	mesh_.draw();
}

//衝突リアクション
void Enemy_Orc::react(Actor& other)
{
	//ダメージ中とダウン中は何もしない
...
//状態更新
void Enemy_Orc::update_state(float delta_time) {
	//各状態に分岐する
	switch (state_) {
...
//ダメージ中
void Enemy_Orc::damage(float delta_time) {
...
//死亡
void Enemy_Orc::down(float delta_time) {
	if (state_timer_ >= mesh_.motion_end_time()) {
		//モーションが終了したら死亡
		die();
	}
...
float Enemy_Orc::target_signed_angle()const {
	//プレイヤーがいなければ何もしない
...
float Enemy_Orc::target_angle() const {
	return std::abs(target_signed_angle());
}

//ターゲットとの距離を求める
float Enemy_Orc::target_distance()const {
	//ターゲットとの距離を計算する
...
//アクターとの衝突処理
void Enemy_Orc::collide_actor(Actor& other) {
	//y座標を除く座標を求める
	GSvector3 position = transform_.position();
	position.y = 0.0f;
...
//状態の変更
void Enemy_Orc::change_state(State state, GSuint motion, bool loop) {
	//モーション番号の更新
	motion_ = motion;
	//モーションループ指定
	motion_loop_ = loop;
...
//アイドル状態
void Enemy_Orc::idle(float delta_time) {
	// 攻撃するか？
	if (is_attack())
	{
		// 攻撃状態に遷移
...
//移動中
void Enemy_Orc::walk(float delta_time) {
	// ターゲット方向の角度を求める
...
//振り向き中
void Enemy_Orc::turn(float delta_time) {
	if (state_timer_ >= mesh_.motion_end_time())
	{
...
//攻撃中
void Enemy_Orc::attack(float delta_time) {
	if (state_timer_ >= mesh_.motion_end_time() * 0.3f && state_timer_ <= mesh_.motion_end_time() * 0.35f)
	{
		//攻撃判定生成
