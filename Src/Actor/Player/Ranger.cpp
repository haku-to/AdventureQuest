...
#include "Collision/PlayerBullet.h"
#include "Assets.h"
#include "Music/SE.h"
...
// モーション番号
enum {
	Motion_Attack2,       //攻撃モーション2
	Motion_Attack1,		  //攻撃モーション1
	Motion_Attack3,		  //攻撃モーション3
...
//体力ゲージサイズ
const GSvector2 TextureHpSize{ 480.0f, 10.0f };

// コンストラクタ
Ranger::Ranger(IWorld* world, const GSvector3& position) :
	mesh_{ Mesh_Ranger, Mesh_Ranger, Mesh_Ranger, Motion_Idle },
	motion_{ Motion_Idle },
	motion_loop_{ true },
	is_running_{ false },
	can_start_{ false },
...
// 更新
void Ranger::update(float delta_time) {
	//状態の更新
	...
	if (Common_Number::is_game_clear_) {
		Common_Number::is_ranger_win_ = true;
		change_state(State::Victory, Motion_Victory, true);
	}
}
// 描画
void Ranger::draw() const {
	// メッシュを描画
	mesh_.draw();
	// 衝突判定のデバッグ表示
	//collider().draw();
...
// 状態の更新
void Ranger::update_state(float delta_time) {
	// 状態遷移
	switch (state_) {
...
// 状態の変更
void Ranger::change_state(State state, GSuint motion, bool loop) {
	// モーション番号の更新
	motion_ = motion;
	// モーションのループ指定
	motion_loop_ = loop;
...
// 移動処理
void Ranger::move(float delta_time) {
	//ダメージを受けてないときだけ攻撃可能
	if (is_damage_ == false)
	{
		// Xボタンで攻撃１
...
// 攻撃中
void Ranger::attack(float delta_time) {
	// 攻撃モーションの終了を待つ
...
// 攻撃中
void Ranger::attack2(float delta_time) {
	//ゲージリセット
	// 攻撃モーションの終了を待つ
...
//回避中
void Ranger::dodge(float delta_time) {
	//回避
	GSvector3 velocity;
...
// ダメージ中
void Ranger::damage(float delta_time) {
	//ノックバックする
...
//死亡中
void Ranger::dead(float delta_time) {
...
// アクターとの衝突処理
void Ranger::collide_actor(Actor& other) {
	// ｙ座標を除く座標を求める
	GSvector3 position = transform_.position();
	position.y = 0.0f;
...
// 攻撃判定の生成
void Ranger::generate_attack_collider(const std::string name, const std::string tag, float delay) {
	// 弾を生成する場所の距離
...
// ターゲット方向の角度を求める（符号付き）
float Ranger::target_angle(GSvector3 target_pos) const {
	// ターゲット方向のベクトルを求める
