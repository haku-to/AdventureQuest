...
enum {
	...
	Motion_ShieldGetHit, //盾ダメージモーション
	Motion_ShieldOn,	 //盾始動モーション
	Motion_ShieldOff,	 //盾終了モーション
	Motion_ShieldNow,	 //盾アイドルモーション
	Motion_Dead,		 //死亡モーション
...
const  GSvector2 UIscale_HPspel{ 0.7f,0.7f };

// コンストラクタ
Knight::Knight(IWorld* world, const GSvector3& position) :
	mesh_{ Mesh_Knight, Mesh_Knight, Mesh_Knight, Motion_Idle },
	motion_{ Motion_Idle },
	motion_loop_{ true },
	is_running_{ false },
	can_start_{ false },
...
void Knight::update(float delta_time) {
	...
	if (Common_Number::is_game_clear_) {
		Common_Number::is_knight_win_ = true;
...
void Knight::draw_gui() const {
	gsDrawSprite2D(UI_HP_Spel, &UIposition_HP_Spel, NULL, NULL, NULL, &UIscale_HPspel, 0.0f);
	//体力ゲージサイズ
...
// 衝突リアクション
void Knight::react(Actor& other) {
	// ダメージ、死亡、防御中の場合は何もしない
	if (state_ == State::Damage || state_ == State::Dead || state_ == State::Dodge || state_ == State::ShieldNow) return;
	// 敵の攻撃判定と衝突したか？
	if (other.tag() == "EnemyAttackTag" || other.tag() == "MageAttackTag") {
		SE::PlaySE(SE_Knight_Damage);
		is_damage_ = true;
		// ターゲット方向のベクトルを求める
		...
		//防御
		if ((state_ == State::ShieldNow || state_ == State::ShieldDamage) &&
			target_angle(other.transform().position()) <= 75.0f) {
			change_state(State::ShieldDamage, Motion_ShieldGetHit, false);
			return;
		}
		//体力減少
...
// 状態の更新
void Knight::update_state(float delta_time) {
	// 状態遷移
	switch (state_) {
	case State::Move:   move(delta_time);   break;
	case State::Attack1: attack(delta_time); break;
	case State::Attack2: attack2(delta_time); break;
	case State::ShieldOn: shield_on(delta_time); break;
	case State::ShieldNow: shield_now(delta_time); break;
	case State::ShieldOff: shield_off(delta_time); break;
	case State::ShieldDamage: hit_shield(delta_time); break;
...
// 状態の変更
void Knight::change_state(State state, GSuint motion, bool loop) {
	// モーション番号の更新
	motion_ = motion;
	// モーションのループ指定
	motion_loop_ = loop;
...
// 移動処理
void Knight::move(float delta_time) {
	//ダメージを受けてないときだけ攻撃可能
	if (is_damage_ == false) {
		// Xボタンで攻撃１
		if (gsGetJoyTrigger(0, GJOY_BUTTON_3)) {
			// 攻撃状態に遷移する
			SE::PlaySE(SE_Knight_Attack1);
	...
	//防御
	if (gsGetJoyTrigger(0, GJOY_BUTTON_5)) {
		change_state(State::ShieldOn, Motion_ShieldOn);
		return;
	}
	//Aボタンで回避
...
// 攻撃中
void Knight::attack(float delta_time) {
	// 攻撃モーションの終了を待つ
	...
	if (state_timer_ >= mesh_.motion_end_time() * 0.2f && can_attack_) {
		generate_attack_collider("PlayerAttack", "PlayerAttackTag", 0.0f);
		can_attack_ = false;
	}
...
// 攻撃中
void Knight::attack2(float delta_time) {
	// 攻撃モーションの終了を待つ
...
//ガード中
void Knight::shield_on(float delta_time) {
	//モーションの終了を待つ
	if (state_timer_ >= mesh_.motion_end_time() * 0.5f) {
		change_state(State::ShieldNow, Motion_ShieldNow);
	}
	//ボタンを離した
	if (gsGetJoyDetach(0, GJOY_BUTTON_5)) {
		change_state(State::ShieldOff, Motion_ShieldOff, false);
	}
}

void Knight::shield_now(float delta_time) {
	//ボタンを離した
	if (gsGetJoyDetach(0, GJOY_BUTTON_5)) {
		change_state(State::ShieldOff, Motion_ShieldOff, false);
	}
...
void Knight::shield_off(float delta_time) {
	//防御
	if (gsGetJoyTrigger(0, GJOY_BUTTON_5)) {
		change_state(State::ShieldOn, Motion_ShieldOn);
		return;
...
void Knight::hit_shield(float delta_time) {
	//ノックバックする
	transform_.translate(velocity_ * delta_time, GStransform::Space::World);
	velocity_ -= GSvector3{ velocity_.x, 0.0f, velocity_.z } *0.5f * delta_time;
	//モーションの終了を待つ
	if (state_timer_ >= mesh_.motion_end_time()) {
		change_state(State::ShieldNow, Motion_ShieldNow, true);
	}

	//ボタンを離した
	if (gsGetJoyDetach(0, GJOY_BUTTON_5)) change_state(State::ShieldOff, Motion_ShieldOff, false);
}

//回避中
void Knight::dodge(float delta_time) {
	////回避
	GSvector3 velocity;
...
// ダメージ中
void Knight::damage(float delta_time) {
	//ノックバックする
	transform_.translate(velocity_ * delta_time, GStransform::Space::World);
	velocity_ -= GSvector3{ velocity_.x, 0.0f, velocity_.z } *0.5f * delta_time;
	// ダメ―ジモーションの終了を待つ
...
//死亡中
void Knight::dead(float delta_time) {
	// ダメ―ジモーションの終了を待つ
	if (state_timer_ >= mesh_.motion_end_time()) {
...
// アクターとの衝突処理
void Knight::collide_actor(Actor& other) {
	// ｙ座標を除く座標を求める
	GSvector3 position = transform_.position();
	position.y = 0.0f;
...
// 攻撃判定の生成
void Knight::generate_attack_collider(const std::string name, const std::string tag, float delay) {
	// 攻撃判定を出現させる場所の距離
...
// ターゲット方向の角度を求める（符号付き）
float Knight::target_angle(GSvector3 target_pos) const {
	// ターゲット方向のベクトルを求める
